...
b: $(@bind b_string TextField(default="1"))``\quad``
h: $(@bind h_string TextField(default="0"))\
\
maximum ``y``-value: $(@bind max_y_string TextField())``\quad``
...
function parse_parameter(text, warn=true)
	text = replace(text, "pi" => "π")
	try
		val = eval(Meta.parse(text))  # yes, this is very unsafe...
		convert(Float64, val)
	catch
		warn && @warn "failed to parse: \"$text\""
		nothing
	end
end;

# ╔═╡ df146996-4a09-4cb6-8793-1c328f750902
remove_nothings(nt::NamedTuple) = NamedTuple{filter(x -> !isnothing(nt[x]), keys(nt))}(nt);

# ╔═╡ ce0441c1-2493-4716-a4de-137c049569ea
begin
	A = parse_parameter(A_string)
	k = parse_parameter(k_string)
	b = parse_parameter(b_string)
	h = parse_parameter(h_string)

	max_y = parse_parameter(max_y_string, false)
	tick_y = parse_parameter(tick_y_string, false)
